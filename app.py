...
def serves():
    beer_names = db.session.query(models.Beer, models.Beer.name).all()
    form = forms.ServingsFormFactory.form(beer_names)
    if form.validate_on_submit():
        return redirect(url_for('/servings/' + form.beer_sel.data))
...
def servings_for(beer_name):
    ...
    results = db.session.query(models.Serves, models.Bar) \
                ...
                .join(models.Bar, models.Serves.bar == models.Bar.name).all()
